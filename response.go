...
type IResponse interface {
	...
	// StatusCode 返回当前请求的http状态码
	StatusCode() int

	// Header 返回当前请求的header信息
	Header() http.Header

	// ContentLength 返回HTTP内容长度
	ContentLength() int64

	// Content 返回HTTP内容
	Content() []byte

	// Resp 返回HTTP包中response的信息
	Resp() *http.Response

	// Request 返回这次请求的request信息
	Request() *http.Request

	// Cookie 根据名称返回cookie值
...
// DefaultBuildResponse 默认的HTTP响应构造器
